          ...
          if actual_step % ops["eval_every"] == 0:
            if (FLAGS.controller_training and
              ...
              for ct_step in range(FLAGS.controller_train_steps *
                ...
                entropy, val_acc = sess.run(run_ops)

                g._unsafe_unfinalize()
                controller_ops["model"].run_trainer(child_ops["model"], 1.)
                g.finalize()

                controller_step, loss, lr, gn, bl, skip, _ = sess.run([
                  controller_ops["model"].train_step,
                  controller_ops["model"].loss,
                  controller_ops["model"].lr, 
                  controller_ops["model"].grad_norm,
                  controller_ops["model"].baseline,
                  controller_ops["model"].skip_rate,
                  controller_ops["model"].train_op
                ])
